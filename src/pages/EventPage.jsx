...
function EventPage() {
  ...
  return (
    ...
    <div className="container mx-auto mt-10 px-4 py-6 bg-primary text-white min-h-screen relative overflow-hidden">
      ...
      {/* Hero Section */}
      <div className="bg-gradient-to-r from-pink-500 via-purple-500 to-indigo-500 py-6 md:py-10 rounded-lg shadow-lg mb-8">
        <h1 className="text-2xl sm:text-4xl font-extrabold text-center text-white tracking-wide animate-fadeIn">
          Find Your Next <span className="text-accent">Event</span>
        </h1>
        <p className="text-center mt-2 text-sm sm:text-lg text-light">
          Browse, search, and plan your perfect day out.
        </p>
      </div>
  
      {/* Search Form */}
      <div className="flex flex-col gap-4 md: md:gap-6 items-center justify-center">
        <SearchEventForm />
      </div>
      ...
      {loading ? (
        ...
        <div className="mt-12">
          <h2 className="text-2xl sm:text-3xl font-bold text-accent mb-6">
            Event Suggestions
          </h2>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 lg:gap-6">
            {eventSuggestions.slice(0, visibleSuggestions).map((event) => (
              ...
              >
                ...
                <p className="text-light text-sm sm:text-base mb-1">
                  <span className="font-semibold">Venue:</span>{" "}
                  {event._embedded?.venues[0]?.name || "Unknown Venue"}
                </p>
                <p className="text-light text-sm sm:text-base mb-2">
                ...
                <Link
                  to={`/event/${event.id}`}
                  state={{ event }}
                  className="block mt-2 text-center px-4 py-2 bg-accent text-white text-sm sm:text-base rounded-lg hover:bg-opacity-80"
                >
                  View Details
                </Link>
              </div>
